...
class PCD8544:
	def __init__(self, spi, cs, dc, rst=None):
		...
		self.chinese1 = chinese1.CN_UTF8()
	...
	def data(self, data):
        ...
        def lcd_write_chinese(self,string,x,y):
		for i in string:
        	    self.position(x,y)
        	    self.data(self.chinese.get_chinese_utf8(i,0))
        	    self.position(x,y+1)
        	    self.data(self.chinese.get_chinese_utf8(i,1))
  		    x=x+8
 	def lcd_write_chinese1(self,string,x,y):
		for i in string:
        	    self.position(x,y)
        	    self.data(self.chinese1.get_chinese_utf8(i,0))
        	    self.position(x,y+1)
        	    self.data(self.chinese1.get_chinese_utf8(i,1))
  		    x=x-8
